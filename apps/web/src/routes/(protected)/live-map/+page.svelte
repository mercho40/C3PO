...
<div class="flex h-full gap-[18px] pb-2">
  ...
  >
    ...
    <!-- Map canvas -->
    <div
      class="relative min-h-0 flex-1 bg-[radial-gradient(ellipse_at_center,#0c1220,#04070d_80%)]"
    >
      <!-- grid lines -->
      <svg
      ...
      >
        <span
          class="absolute -inset-7 rounded-full bg-[rgba(159,197,255,0.14)] blur-[7px]"
        ></span>
        <span
          class="absolute -inset-3.5 rounded-full border {online
            ? 'border-[#9ae5f8]'
            : 'border-[#ff4d6a]'} opacity-50"
        ></span>
        <!-- heading wedge rotates with yaw -->
        <span
          class="absolute top-1/2 left-1/2 origin-bottom -translate-x-1/2 -translate-y-full"
          style="transform:translate(-50%,-100%) rotate({yawDeg}deg)"
        >
          <span class="block h-4 w-px bg-[#9ae5f8]"></span>
        </span>
        <span
          class="relative block size-4 rounded-full {online
            ? 'bg-[#9ae5f8]'
            : 'bg-[#ff4d6a]'} shadow-[0px_0px_24px_#7ee5ff,0px_0px_0px_3px_rgba(5,7,13,0.85)]"
        ></span>
        <span
          class="absolute -top-7 left-1/2 -translate-x-1/2 rounded-md border border-[rgba(180,210,255,0.18)] bg-[rgba(5,7,13,0.85)] px-2 py-1 font-mono text-[10px] tracking-wide whitespace-nowrap text-[#eaf1ff] backdrop-blur-sm"
          >{unit.id}</span
        >
      </div>
      ...
      <!-- Compass -->
      <div
        class="absolute top-5 right-5 flex size-[60px] flex-col items-center justify-center rounded-full border border-[rgba(180,210,255,0.18)] bg-[rgba(5,7,13,0.6)] backdrop-blur-sm"
      >
        <Navigation
          class="size-3.5 text-[#7ee5ff] transition-transform duration-500"
          style="transform:rotate({yawDeg}deg)"
        />
        <span class="font-mono text-[10px] tracking-wide text-[#9fc5ff]"
          >{yawDeg}°</span
        >
      </div>
      ...
      <div class="absolute bottom-5 left-5 flex flex-col gap-1">
        ...
        <span class="font-mono text-[10px] text-[#8a96ad]"
          >{(100 / scale / 2).toFixed(1)} m</span
        >
      </div>

      <!-- Zoom -->
      <div
        class="absolute right-5 bottom-5 flex flex-col overflow-hidden rounded-lg border border-[rgba(180,210,255,0.18)] bg-[rgba(5,7,13,0.6)] backdrop-blur-sm"
      >
        <Button
          variant="ghost"
          size="icon"
  ...
  <!-- Units panel -->
  <aside
    class="flex w-[340px] shrink-0 flex-col gap-2.5 rounded-[14px] border border-[rgba(180,210,255,0.08)] bg-gradient-to-b from-[#0c1220] to-[#121828] p-3.5"
  >
    <div
    ...
    >
      <div class="flex items-center gap-2.5">
        <span
          class="flex items-center rounded-full border border-[rgba(180,210,255,0.08)] bg-[rgba(180,210,255,0.04)] px-1.5 py-1"
        >
          <span
            class="size-1.5 rounded-sm {unit.active
              ? 'bg-[#5ee7a1] shadow-[0px_0px_10px_rgba(94,231,161,0.6)]'
              : 'bg-[#ff4d6a] shadow-[0px_0px_10px_rgba(255,77,106,0.6)]'}"
          ></span>
        </span>
        <span class="font-mono text-xs tracking-wide text-[#eaf1ff]"
          >{unit.id}</span
        >
        <span
          class="ml-auto font-mono text-[9px] tracking-[0.09em] text-[#8a96ad] uppercase"
          >{unit.tag}</span
        >
      </div>
      ...
      <div class="flex items-center gap-2">
        <div
          class="h-1 flex-1 overflow-hidden rounded-full bg-[rgba(180,210,255,0.08)]"
        >
          <div
        ...
        <span class="font-mono text-[11px] text-[#eaf1ff]"
          >{robot?.battery_pct != null ? `${unit.battery}%` : "—"}</span
        >
      </div>
    </div>
    <p
      class="mt-1 font-mono text-[9px] leading-relaxed tracking-wide text-[#8a96ad]"
    >
      Posición en tiempo real desde <span class="text-[#9fc5ff]">/state</span>.
      El origen (0, 0) está en el centro; el rastro integra el recorrido a
